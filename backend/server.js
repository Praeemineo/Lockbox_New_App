...
let apiFields = [
    ...
    { fieldId: "FLD-002", fieldName: "LockboxBatchDestination", necessity: "Mandatory", fieldType: "Constant", dataType: "String", maxLength: 10, defaultValue: "SAMPLEDEST", description: "Destination for lockbox batch (10 chars max)", isEditable: true, createdAt: new Date().toISOString() },
    { fieldId: "FLD-003", fieldName: "LockboxBatchOrigin", necessity: "Mandatory", fieldType: "Constant", dataType: "String", maxLength: 10, defaultValue: "1234567890", description: "Origin of lockbox batch (10 chars max)", isEditable: true, createdAt: new Date().toISOString() },
    ...
    { fieldId: "FLD-013", fieldName: "PaymentReference", necessity: "Optional", fieldType: "User Input", dataType: "String", maxLength: 50, description: "Payment reference/Invoice number", isEditable: false, createdAt: new Date().toISOString() },
...
const DEFAULT_REFERENCE_DOC_RULES = [
    {
        ...
        description: 'Use Accounting Document number (BELNR) as reference',
        ruleType: 'BELNR',
        logicCondition: 'lbinvref = belnr',
        documentIdType: 'Accounting document',
        priority: 1,
        isDefault: false,
        active: true,
        createdAt: new Date().toISOString()
    },
    {
        id: uuidv4(),
        ruleId: 'RULE-002',
        ruleName: 'Reference Document Number',
        description: 'Use Invoice Reference number (XBLNR) as reference',
        ruleType: 'XBLNR',
    ...
    },
    {
        ...
        description: 'Try Accounting Document first, if not found use Invoice Reference',
    ...
    },
    {
        ...
        description: 'Try Invoice Reference first, if not found use Accounting Document',
    ...
    for (const [checkKey, checkData] of Object.entries(checkGroups)) {
        ...
        if (checkData.invoices && checkData.invoices.length > 0) {
            ...
            const clearingResults = checkData.invoices.map(inv => {
                ...
                console.log(`    Rule evaluation: InvoiceNumber=${invoiceNumber}, XBLNR=${xblnr}, BELNR=${belnr}`);
                ...
                const clearing = {
                    // PaymentReference determined by active reference document rule
...
app.post('/api/lockbox/process', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < extractedData.length; i++) {
            ...
            // Check mandatory fields
            if (!row.CheckNumber) {
                warnings.push(`Row ${i + 1}: Missing Check Number (will be auto-generated)`);
            }
            if (!row.InvoiceNumber) {
                warnings.push(`Row ${i + 1}: Missing Invoice/Payment Reference`);
            }
            if (row.InvoiceAmount <= 0 && row.CheckAmount <= 0) {
                warnings.push(`Row ${i + 1}: Zero or negative amount`);
            }
        }
        
        // Validate pattern-specific rules
        if (patternResult.analysis.hasEmptyCheckRows && patternResult.pattern.patternType !== 'SINGLE_CHECK_MULTI_INVOICE') {
