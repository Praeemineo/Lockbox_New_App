        ...
        try {
            ...
            if (lockboxBatchInternalKey && lockboxBatch) {
                try {
                    ...
                    // Extract customer from customerMapping (NOT from payload to avoid SAP rejecting '_' properties)
                    // Customer mapping is stored separately during payload build
                    // Fallback to extractedRows if mapping is not available (backward compatibility)
                    const firstItem = payload.to_Item?.results?.[0];
                    const customerFromPayload = header.customer_mapping?.['1'] || extractedRows[0]?.Customer || '';
                    
                    console.log('Customer from mapping:', customerFromPayload);
    ...
    try {
        ...
        */
        ...
        documents.forEach((doc, idx) => {
            ...
            console.log(`   🏢 Sending Bank Field: ${doc.SendingBank || 'N/A'}`);
            ...
            console.log(`   📌 Bank Statement Item: ${doc.BankStatementItem || 'N/A'}`);
...
function buildStandardPayload(extractedData, lockboxId, runId) {
    ...
    // Customer mapping - SEPARATE from payload to avoid SAP rejecting properties starting with '_'
    // Maps itemId to customer number for later use in GET LockboxClearing API
    ...
    for (const [checkKey, checkData] of Object.entries(checkGroups)) {
        ...
        // Store customer for GET LockboxClearing API call (NOT sent to SAP POST)
        ...
        // Store customer in SEPARATE mapping to avoid SAP rejecting properties starting with '_'
        customerMapping[itemId] = checkData.customer;
...
app.post('/api/lockbox/process', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        const { payload, customerMapping } = buildStandardPayload(processedData, lockboxId, run.runId);
        run.sapPayload = payload;
        run.customerMapping = customerMapping; // Store customer mapping separately
...
app.post('/api/lockbox/runs/:runId/simulate', async (req, res) => {
    ...
    // Use customer from customerMapping (separate from payload to avoid SAP rejecting '_' properties)
    const payloadItems = sapPayload?.to_Item?.results || [];
    const customerMapping = run.customerMapping || {};  // Retrieve customer mapping from run object
    const mockBatchItems = [];
    for (let i = 1; i <= itemCount; i++) {
        const mockPA = mockPaymentAdvice.slice(0, -1) + i;
        const payloadItem = payloadItems[i - 1] || {};
        // Customer from file (stored in customerMapping during payload build, NOT in payload itself)
        const customerFromFile = customerMapping[i] || extractedRows[i - 1]?.Customer || customerNumber;
    ...
    try {
        ...
        if (useMock) {
            ...
            try {
                ...
                // Get customer numbers from customerMapping for GET LockboxClearing
                // PaymentAdviceAccount = Customer number (from uploaded file)
                // Customer mapping is stored separately to avoid SAP rejecting '_' properties
                const payloadItems = sapPayload?.to_Item?.results || [];
                ...
                for (let i = 1; i <= itemCount; i++) {
                    ...
                    const itemIdx = i - 1;
                    // Get customer from customerMapping (NOT from payload to avoid SAP rejecting '_' properties)
                    const payloadItem = payloadItems[itemIdx] || {};
                    const customerFromFile = customerMapping[i] || extractedRows[itemIdx]?.Customer || '';
