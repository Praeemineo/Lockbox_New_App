...
,
        ...
        _showRuleExecutionResultsDialog: function (oRun) {
            ...
            if (!oSummary || (!oSummary.rule001 && !oSummary.rule002)) {
                return; // No results to show
            }
            
            var sContent = "";
            sContent += "<div style='font-family: Arial, sans-serif; padding: 10px;'>";
            sContent += "<h3 style='color: #0854A0; margin-bottom: 15px;'>✅ Validation & Mapping Complete</h3>";
            sContent += "<p style='margin-bottom: 20px;'>Processing rules have been executed successfully. Below are the derived values:</p>";
            
            // RULE-001 Results
            if (oSummary.rule001 && oSummary.rule001.executed) {
                sContent += "<div style='background: #f5f5f5; padding: 15px; margin-bottom: 15px; border-left: 4px solid #0854A0;'>";
                sContent += "<h4 style='margin: 0 0 10px 0; color: #0854A0;'>📄 RULE-001: Accounting Document Lookup</h4>";
                sContent += "<p style='margin: 5px 0;'><strong>Records Enriched:</strong> " + oSummary.rule001.recordsEnriched + "</p>";
                sContent += "<p style='margin: 5px 0 10px 0;'><strong>Target Field:</strong> Paymentreference (BELNR)</p>";
                
                if (oSummary.rule001.sampleValues && oSummary.rule001.sampleValues.length > 0) {
                    sContent += "<table style='width: 100%; border-collapse: collapse; margin-top: 10px;'>";
                    sContent += "<thead><tr style='background: #0854A0; color: white;'>";
                    sContent += "<th style='padding: 8px; text-align: left;'>Invoice Number</th>";
                    sContent += "<th style='padding: 8px; text-align: left;'>Derived BELNR (Paymentreference)</th>";
                    sContent += "</tr></thead><tbody>";
                    
                    oSummary.rule001.sampleValues.forEach(function(row, idx) {
                        var bgColor = idx % 2 === 0 ? '#ffffff' : '#f9f9f9';
                        sContent += "<tr style='background: " + bgColor + ";'>";
                        sContent += "<td style='padding: 8px; border-bottom: 1px solid #ddd;'>" + (row.invoiceNumber || 'N/A') + "</td>";
                        sContent += "<td style='padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold; color: #0854A0;'>" + (row.derivedBELNR || 'N/A') + "</td>";
                        sContent += "</tr>";
                    });
            ...
            // RULE-002 Results
            if (oSummary.rule002 && oSummary.rule002.executed) {
                sContent += "<div style='background: #f5f5f5; padding: 15px; margin-bottom: 15px; border-left: 4px solid #107E3E;'>";
                sContent += "<h4 style='margin: 0 0 10px 0; color: #107E3E;'>🏦 RULE-002: Partner Bank Details</h4>";
                sContent += "<p style='margin: 5px 0;'><strong>Records Enriched:</strong> " + oSummary.rule002.recordsEnriched + "</p>";
                sContent += "<p style='margin: 5px 0 10px 0;'><strong>Target Fields:</strong> PartnerBank (BANKS), PartnerBankAccount (BANKL), PartnerBankCountry (BANKN)</p>";
                
                if (oSummary.rule002.sampleValues && oSummary.rule002.sampleValues.length > 0) {
                    sContent += "<table style='width: 100%; border-collapse: collapse; margin-top: 10px;'>";
                    sContent += "<thead><tr style='background: #107E3E; color: white;'>";
                    sContent += "<th style='padding: 8px; text-align: left;'>Customer Number</th>";
                    sContent += "<th style='padding: 8px; text-align: left;'>Partner Bank</th>";
                    sContent += "<th style='padding: 8px; text-align: left;'>Bank Account</th>";
                    sContent += "<th style='padding: 8px; text-align: left;'>Country</th>";
                    sContent += "</tr></thead><tbody>";
                    
                    oSummary.rule002.sampleValues.forEach(function(row, idx) {
                        var bgColor = idx % 2 === 0 ? '#ffffff' : '#f9f9f9';
                        sContent += "<tr style='background: " + bgColor + ";'>";
                        sContent += "<td style='padding: 8px; border-bottom: 1px solid #ddd;'>" + (row.customerNumber || 'N/A') + "</td>";
                        sContent += "<td style='padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold; color: #107E3E;'>" + (row.partnerBank || 'N/A') + "</td>";
                        sContent += "<td style='padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold; color: #107E3E;'>" + (row.partnerBankAccount || 'N/A') + "</td>";
                        sContent += "<td style='padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold; color: #107E3E;'>" + (row.partnerBankCountry || 'N/A') + "</td>";
                        sContent += "</tr>";
                    });
            ...
            sContent += "<p style='margin-top: 20px; color: #666; font-size: 12px;'>💡 <em>These values have been derived from SAP and will be used in the final payload.</em></p>";
            sContent += "</div>";
            
            // Create and show dialog
            if (!that._oRuleResultsDialog) {
                that._oRuleResultsDialog = new sap.m.Dialog({
                    title: "Rule Execution Results - Validate & Map Stage",
                    contentWidth: "800px",
                    contentHeight: "600px",
                    ...
                    beginButton: new sap.m.Button({
                        text: "View Field Mapping",
                        press: function () {
                            that._oRuleResultsDialog.close();
                            // Navigate to field mapping preview
                            MessageToast.show("Check the 'Preview Field Mapping' section for complete payload details");
                        }
                    }),
                    endButton: new sap.m.Button({
                        text: "Close",
                ...
                that.getView().addDependent(that._oRuleResultsDialog);
            } else {
                // Update content if dialog already exists
                that._oRuleResultsDialog.destroyContent();
                that._oRuleResultsDialog.addContent(new sap.ui.core.HTML({
                    content: sContent
                }));
            }
            
            that._oRuleResultsDialog.open();
        }
